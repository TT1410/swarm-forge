  ...
  (let [root (tmp-dir)
    ...
    (try
      ...
      (let [result (run {:dir root
        ...
        (is (fs/exists? (fs/path root ".worktrees/merger-003"))
            "merge-blocked merger worktrees are preserved for recovery")
        (is (str/includes? (:out result) "PRESERVED_FOR_RECOVERY:")
            "teardown reports intentionally preserved worktrees")
        ...
        (is (git-worktree-registered? root (fs/path root ".worktrees/merger-003")))
        (is (not (git-branch-exists? root "swarmforge-code-reviewer-001")))
        (is (git-branch-exists? root "swarmforge-merger-003"))

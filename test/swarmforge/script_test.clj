  ...
  (let [root (tmp-dir)]
    (try
      (write-file (fs/path root ".swarmforge/roles.tsv")
                  (format "specifier\tmaster\t%s\tsession\tSpecifier\tcodex\ttask\n" root))
      (let [missing (run {:dir root :ok? false}
                         (script "swarm_tool.sh") "require" "crap4js")
            help (run {:dir root :ok? false} (script "swarm_tool.sh") "--help")
            listed (str (:err help) (:out help))]
        (is (not= 0 (:exit missing)))
        (is (str/includes? (:err missing) "MISSING: crap4js"))
        (is (not (str/includes? (:err missing) "Unknown tool")))
        (doseq [tool ["vitest" "jscpd" "crap4js" "mutate4js"]]
          (is (str/includes? listed tool))))
      (finally
        (fs/delete-tree root)))))
...
(deftest mutate4js-wrapper-is-differential-with-four-workers
  ;; Given an installed mutate4js wrapper
  ;; When it is invoked with --mutate-all
  ;; Then --mutate-all is dropped and --max-workers 4 is used,
  ;; while no-run manifest modes keep their own shape
  (let [root (tmp-dir)
        prefix (write-fake-node-prefix! (fs/path root "node") "mutate4js")]
    (try
      (write-file (fs/path root ".swarmforge/roles.tsv")
                  (format "specifier\tmaster\t%s\tsession\tSpecifier\tcodex\ttask\n" root))
      (run {:dir root :env (node-env prefix)} (script "swarm_tool.sh") "ensure" "mutate4js")
      (let [wrapper (str (fs/path root ".swarmforge/bin/mutate4js"))
            scored (:out (run {:dir root} wrapper "src/a.ts"
                              "--lcov" "coverage/lcov.info" "--mutate-all"))
            scan (:out (run {:dir root} wrapper "src/a.ts" "--scan"))

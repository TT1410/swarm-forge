...
- `node` 22 or newer and `npm`, for projects whose language is JavaScript or
  TypeScript
...
| JavaScript/TypeScript | Vitest, `@vitest/coverage-v8` | Stryker | `crap4js` | `jscpd` |
...
JavaScript projects split their tools. Vitest and `@vitest/coverage-v8` are
imported by the project's own config and test files, so they live in the
project's `package.json` like any dev dependency. Stryker, `crap4js`, and
`jscpd` are standalone commands and come from npm into a project-local prefix
at `.swarmforge/node`, so a pack never adds them to the project. Stryker runs
through its command runner and keeps its differential manifest at
`reports/stryker-incremental.json`; its dedicated Vitest runner currently
fails against Vitest 5 and reports every mutant as survived.

`crap4js` is a prerelease licensed **CC-BY-NC-4.0**, an original work inspired
by `crap4clj` rather than a port of it. That licence forbids commercial use,
so check it against the project before letting a pack run the CRAP gate.

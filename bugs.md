...
3. Multiple hardener assignments completed, but no QA batch started because the
   stories still lacked `hardener_sha`.
...
6. Batch status files can remain open after the relevant assignments have been
   merged.
7. Because the phase SHA is missing, the FSM does not consider the story
...
3. Story 6's accepted QA procedure review r2 was merged at merge commit
   `60853bcfc0`, but the story packet remained
...
6. This repair action should run before downstream eligibility checks so
   stories cannot remain blocked by a merged-but-unrecorded review.

### Architecture Review Flow Must End The Story

The architect is a review role at the end of the story workflow. Architecture
output should either finish the story or send it through one senior implementer
repair step and then finish it.

Expected behavior:

1. Non-blocking architecture recommendations must not be treated as
   `changes-requested`.
2. Senior implementer should be triggered only by an explicit blocking
   `changes-requested` architecture disposition.
3. Accepted architecture with optional recommendations should mark the story
   done after any configured architecture/final approval gates are satisfied.
4. If architecture requests blocking changes, the story should route through
   senior implementer and then be marked done after any configured final
   approval gate is satisfied.
5. The intended architecture-stage flow is:
...
leaving `.squad/agents/<agent>/status` and assignment status files in active
states.
...
4. Assignment status files still showed active work after kill, including

...
1. **Rejecting A Story Approval Should Create A Blocker**
...
2. **Blocker UI Should Offer Re-Enter Pipeline (Design Later)**
   ...
   detailed design until later.

3. **Same Assignment Can Spawn Two Live Agents**
...
4. **Parallel Mergers Thrash On Shared Product Files**

   Observed in `~/junk/squad`: many parallel implementers and then many parallel
   mergers hit dry-run merge failures into SL `master`, especially on
   `src/wumpus/core.clj` and tests. In one run, at least eight distinct merger
   handoffs were `merge_blocked` when landing back to SL (including second-level
   `*-merge-merge` attempts); only a minority of merger assignments fully
   succeeded. Nested recovery (r2 implementer, r2 merger, merge-merge) stacked
   quickly.

   Mitigation note (likely high leverage, not a full cure): treat **merger as a
   singleton** (like hardener / qa / architect) so only one merger integrates
   against current `HEAD` at a time. That should avoid much of the
   merger-vs-merger cascade and nested re-merge thrashing. It does **not** by
   itself prevent the first-wave implementer conflicts from parallel stories
   sharing the same core files; serializing or batching implementers when they
   touch the same paths may still be needed.

5. **All Stories Should Have A Story Number**
...
6. **Agents Can Report Retired Before Workflow Resolves Their Handoff**
...
7. **squadd.log Concurrent Writes Corrupt Event Lines**
...
8. **Teardown Leaves Assignments `in_progress` With Dead Agents**
...
9. **`merge-merge` Max Depth Sometimes Hard-Blocks Instead Of Recovering**
...
10. **Merge Thrash Includes Tooling Config, Not Only Product Code**
    ...
    **r6**, cleaner ladders up to **r5**, 40 merge-blocked rejection notes).
    ...
    path so they do not drive endless rN rework. Complements bug 4 (singleton
    merger / shared-path serialization).

11. **Merge-Ready Dry-Run Often Runs Twice Before Reject**
...
12. **Merge-Conflict Rejects Do Not Create Dashboard Blockers**
    ...
    Related to bug 1 (approval reject → blocker) but for the **merge-conflict
